...
public class DNASequence {
	...
	public boolean isProtein() {
		return this.alphabet.contains("protein");
	}
	...
	public boolean contains(DNASequence s2) {
		boolean return1 = this.sequence.contains(s2.sequence);
		return return1;
	}
	...
	public DNASequence append(DNASequence s2) {
		return new DNASequence(this.sequence.concat(s2.sequence),this.alphabet);
	}
	...
	public static DNASequence longestSequence(DNASequence[] sequences) {
		DNASequence longS= sequences[0];
		if(sequences.length==0)
			return null;
		else{

			for(DNASequence s: sequences)
				if(s==null)
					break;
				else if(s.sequence.length()> longS.sequence.length())
					longS = s; 
		}
		return longS;
	}
	...
	public static DNASequence concatenate(DNASequence[] list) {
		DNASequence newS = new DNASequence("","dna");
		if(list.equals(null))
			return null;
		if(list.length==0)
	...
	public boolean exists(DNASequence[] list) {
		for(DNASequence s: list) {
			if(s==null)
				break;
			if(this.sequence.equals(s.sequence))
				return true;
		}
...
//		for(DNASequence anListA: list) {
//			for(DNASequence anListB: list) {
//				if(anListA == null || anListB == null) {break;}
//				if(anListA.contains(anListB) || anListB.contains(anListA)) {return true;}
//			}
//		}
